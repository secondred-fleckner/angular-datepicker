    ...
    var module = angular.module('720kb.datepicker', []),
            ...
            return [
                '<div class="_720kb-datepicker-calendar-header">',
                  '<div class="_720kb-datepicker-calendar-header-left">',
                    '<a class="_720kb-datepicker-calendar-month-button" ng-class="{\'_720kb-datepicker-item-hidden\': !willPrevMonthBeSelectable()}" ng-click="prevMonth();$event.stopPropagation()" title="{{ buttonPrevTitle }}">',
                      prevButton,
                    '</a>',
                  '</div>',
                  '<div class="_720kb-datepicker-calendar-header-middle-left _720kb-datepicker-calendar-month">',
                    '<a ng-click="paginateMonths(year); showYearsPagination = false; showMonthsPagination = !showMonthsPagination;$event.stopPropagation()">',
                      '<span>',
                  ...
                  '<div class="_720kb-datepicker-calendar-header-middle-right _720kb-datepicker-calendar-month">',
                    '<a ng-click="paginateYears(year); showMonthsPagination = false; showYearsPagination = !showYearsPagination;$event.stopPropagation()">',
                      '<span>',
                        '{{year}}',
                  ...
                  '<a class="_720kb-datepicker-calendar-month-button" ng-class="{\'_720kb-datepicker-item-hidden\': !willNextMonthBeSelectable()}" ng-click="nextMonth();$event.stopPropagation()" title="{{ buttonNextTitle }}">',
                    nextButton,
                  '</a>',
                  '</div>',
                '</div>'
        ...
        return [
            '<div class="_720kb-datepicker-calendar-header grey" ng-show="showMonthsPagination">',
              '<div class="_720kb-datepicker-calendar-years-pagination">',
                '<a ng-class="{\'_720kb-datepicker-active\': m.number === monthNumber, \'_720kb-datepicker-disabled\': !isSelectableMaxMonth(m.number, year) || !isSelectableMinMonth(m.number, year)}" ng-click="setNewMonth(m.number, year);$event.stopPropagation()" ng-repeat="m in paginationMonths track by $index">',
        ...
        return [
            '<div class="_720kb-datepicker-calendar-header grey" ng-show="showYearsPagination">',
              '<div class="_720kb-datepicker-calendar-years-pagination">',
                '<a ng-class="{\'_720kb-datepicker-active\': y === year, \'_720kb-datepicker-disabled\': !isSelectableMaxYear(y) || !isSelectableMinYear(y)}" ng-click="setNewYear(y);$event.stopPropagation()" ng-repeat="y in paginationYears track by $index">',
                  '{{y}}',
                '</a>',
              '</div>',
              '<div class="_720kb-datepicker-calendar-years-pagination-pages">',
                '<a ng-click="paginateYears(paginationYears[0]);$event.stopPropagation()" ng-class="{\'_720kb-datepicker-item-hidden\': paginationYearsPrevDisabled}">',
                  prevButton,
                '</a>',
                '<a ng-click="paginateYears(paginationYears[paginationYears.length -1 ]);$event.stopPropagation()" ng-class="{\'_720kb-datepicker-item-hidden\': paginationYearsNextDisabled}">',
                  nextButton,
                '</a>',
              '</div>',
            '</div>'
        ...
        generateDays = function generateDays() {
            return [
                ...
                '{{px}}',
                '</a>',
                '<a ng-repeat="item in days" ng-click="setDatepickerDay(item);$event.stopPropagation()" ng-class="{\'new-month-container\': (monthCountStart | isMonthCountEdge:monthNumber:year:item:0:datepickerHidePms),\'_720kb-datepicker-active\': selectedDay === item && selectedMonth === monthNumber && selectedYear === year, \'_720kb-datepicker-disabled\': !isSelectableMinDate(year + \'/\' + monthNumber + \'/\' + item ) || !isSelectableMaxDate(year + \'/\' + monthNumber + \'/\' + item) || !isSelectableDate(monthNumber, year, item) || !isSelectableDay(monthNumber, year, item),\'_720kb-datepicker-today\': item === today.getDate() && monthNumber === (today.getMonth() + 1) && year === today.getFullYear()}" class="_720kb-datepicker-calendar-day">',
        ...
        generateOptions = function generateOptions() {
            return [
                ...
                '<a ng-show="isTodayVisible && (!customButtons || customButtons.length <= 0)" ng-click="resetDatepickerValue(true);$event.stopPropagation()">',
                '{{"Today"|getTranslationValue:translations}}',
                '</a>',
                '<ul ng-show="customButtons && customButtons.length > 0">',
                '<li class="emdesk-tagcloud" ng-repeat="customButton in customButtons" ng-click="setCustomButtonValue(customButton);$event.stopPropagation()">',
                '<span class="tagcloud custom-buttons">{{customButton.label}}</span>',
                '</li>',
                '</ul>',
                '<a ng-click="resetDatepickerValue();$event.stopPropagation()">',
        ...
        function datepickerDirective($window, $compile, $locale, $filter, $interpolate, $timeout, datepickerConfig, datepickerSettings) {
            return {
                ...
                'link': function link($scope, element, attr) {
                    ...
                    if (angular.isDefined($scope.ngModel) && $scope.dateMinLimit && !$scope.isSelectableMinYear($scope.year) || !$scope.isSelectableMinDate($scope.year + '/' + $scope.monthNumber + '/' + $scope.day)) {
                        resetToMinDate();
                    }
                    if (angular.isDefined($scope.ngModel) && $scope.dateMaxLimit && !$scope.isSelectableMaxYear($scope.year) || !$scope.isSelectableMaxDate($scope.year + '/' + $scope.monthNumber + '/' + $scope.day)) {
                        resetToMaxDate();
                    }
